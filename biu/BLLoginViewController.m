...
- (void)viewDidLoad {
    ...
    _btnForgotPw.titleLabel.font = [UIFont fontWithName:@"ArialMT" size:15];
    ...
    _lbLoginWith.text = NSLocalizedString(@"OR LOG IN WITH", nil);
    ...
    [_btnForgotPw mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_tfPassword.mas_bottom).with.offset(21.2);
        make.centerX.equalTo(self.view.mas_centerX);
    }];
    ...
    [_lbLoginWith mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(_btnLoginWithTwitter.mas_top).with.offset(-13.0);
        make.left.equalTo(self.view).with.offset(21.0);
        make.right.equalTo(self.view).with.offset(-21.0);
    }];
